...
class UserAPIPretask extends UserAPIBasic {
  ...
  private function getPretask($QUERY) {
    ...
    $response = [
      ...
      UResponseTask::PMSN_COUNT => $pretask->getPmsnCount(),
      UResponseTask::PMSN_ADR => $pretask->getPmsnAdr(),
      ...
      UResponseTask::HASHLIST_ID => $pretask->getHashlistId(),
      UResponseTask::FILES => $pretask->getFiles(),
      UResponseTask::SPEED => $pretask->getSpeed(),
      UResponseTask::SEARCHED => $pretask->getSearched(),
      UResponseTask::CHUNKIDS => $pretask->getChunkIds(),
      UResponseTask::AGENTS => $pretask->getAgents(),
      UResponseTask::IS_COMPLETE => $pretask->getIsComplete()
    ];
